  ...
  mp_vectort &dest)
{
  ...
  else if(expr.id()==ID_ror)
  {
    ...
    if(op.size()==1 && distance.size()==1)
    {
      mp_integer final=rotate_right(op.front(),
        distance.front(),
  ...
  else if(expr.id()==ID_rol)
  {
    ...
    if(op.size()==1 && distance.size()==1)
    {
      mp_integer final=rotate_left(op.front(),
        distance.front(),
  ...
  else if(expr.id()==ID_div)
  {
    ...
    evaluate(div_expr.op0(), dividend);
    evaluate(div_expr.op1(), divisor);
  ...
  else if(expr.id()==ID_typecast)
  {
    ...
    if(casted.size()==1)
    {
      ...
      {
        const std::string s=
          integer2binary(value, to_signedbv_type(typecast_expr.type()).get_width());
      ...
      {
        const std::string s=
          integer2binary(value, to_unsignedbv_type(typecast_expr.type()).get_width());
        dest.push_back(binary2integer(s, false));
        return;
      }
      else if((typecast_expr.type().id()==ID_bool) || (typecast_expr.type().id()==ID_c_bool))
      {
  ...
  bool fail_quietly)
{
          ...
          expr.id()==ID_byte_extract_big_endian)
  {
    ...
    {
      mp_integer memory_offset;
      if(!byte_offset_to_memory_offset(byte_extract_expr.op().type(),
        extract_offset[0], memory_offset))
        return evaluate_address(byte_extract_expr.op(), fail_quietly)+memory_offset;
    }
  }
  else if(expr.id()==ID_if)
  {
